...
export class MySQLBuilderService {
  ...
  private async buildTableQuery(tableName: string, tableQuery: TableQuery): Promise<Query> {
    this.logger.debug(`构建表查询: ${tableName}, 操作: ${tableQuery.operation}`);

    let sql = '';
    let params: any[] = [];

    // 根据操作类型构建 SQL
    switch (tableQuery.operation) {
      ...
      default:
        throw new Error(`不支持的表操作类型: ${tableQuery.operation}`);
    }
  ...
  private buildSelectQuery(tableName: string, tableQuery: TableQuery): { sql: string; params: any[] } {
    ...
    // 构建 WHERE 子句
    const whereResult = this.buildWhereClause(tableQuery.where);
  ...
  private buildWhereClause(where: any): { where: string; params: any[] } {
    ...
    const params: any[] = [];
    const conditions = this.buildConditions(where, params);
   ...
   * 构建条件
   */
  private buildConditions(conditions: any, params: any[]): string[] {
    const result: string[] = [];
